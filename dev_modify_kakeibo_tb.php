<?php
    ...
    if ($_SESSION['user_id'] != '' && !$_GET['execution']) {
        ...
        $sql = sprintf("SELECT * FROM dev_kakeibo WHERE message_id = '%s' AND id = '%s' ",
        ...
        $res = mysqli_query($db_link, $sql);
        if ($res) {
            $redirect_url = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'] . '/dev_home.php?modify_result=true';
        } else {
            $redirect_url = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'] . '/dev_home.php?modify_result=false';
        }
...
?>
    ...
    <script>
        function backbrowser() {
            var url = 'https://st0731-dev-srv.moo.jp/dev_home.php';
