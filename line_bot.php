<?php
    ...
    function update_kakeibo_classify($db_link, $user_id, $group_id, $message_text, $ch_type) {
        if ($ch_type == 'user') {
            $sql = sprintf("UPDATE kakeibo SET classify_id = '%s' WHERE id = '%s' AND groupId = '' ORDER BY insert_time DESC Limit 1",
                mysqli_real_escape_string($db_link, $message_text),
                mysqli_real_escape_string($db_link, $user_id)
            );
        } else {
            $sql = sprintf("UPDATE kakeibo SET classify_id = '%s' WHERE id = '%s' AND groupId = '%s' ORDER BY insert_time DESC Limit 1",
                mysqli_real_escape_string($db_link, $message_text),
                mysqli_real_escape_string($db_link, $user_id),
                mysqli_real_escape_string($db_link, $group_id)
            );
    ...
    function insert_group_name($db_link, $group_id, $group_name) {
        $sql = sprintf("INSERT INTO group_name (groupId, group_name) VALUES ('%s', '%s')",
            mysqli_real_escape_string($db_link, $group_id),
    ...
    function delete_group_name($db_link, $group_id) {
        $sql = sprintf("DELETE FROM group_name WHERE groupId = '%s' limit 1",
            mysqli_real_escape_string($db_link, $group_id)
        );
    ...
    if ($message_text == 'いくら' || $message_text == '幾ら') {
        ...
        $return_message_text .=  "https://st0731-dev-srv.moo.jp/index.php";
