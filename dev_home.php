<?php
    ...
    if ($_SESSION['access_token'] != '') {
        ...
        if ($_GET['delete'] != '') {
            ...
            //論理削除
            $sql = sprintf("UPDATE dev_kakeibo SET id = '', groupId = '' WHERE message_id = '%s' AND id = '%s' Limit 1",
    ...
    $sql = sprintf("SELECT * FROM dev_kakeibo WHERE id = '%s' ",
        mysqli_real_escape_string($db_link, $user_id)
    );
    ...
    //グループidからグループ名を取得
    $sql = 'SELECT groupId, group_name FROM dev_group_name WHERE groupId IN (' . $in_pattern . ')';
    $group_name_array = [];
    $res = mysqli_query($db_link, $sql);
    while ($row = mysqli_fetch_assoc($res)) {
...
?>
    ...
    <script>
        function ModifyClick(event) {
            var url = 'https://st0731-dev-srv.moo.jp/dev_modify_kakeibo_tb.php?modify=' + event;
    ...
    <script>
        function DeleteClick(event) {
            ...
            if( res == true ) {
                // OKなら移動
                var url = 'https://st0731-dev-srv.moo.jp/dev_home.php?delete=' + event;
                window.location.href = url;
            } else {
                // キャンセルならアラートボックスを表示
                alert("キャンセルしました");
